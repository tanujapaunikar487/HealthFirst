...
**Detail pages**: `DetailRow` (`Components/ui/detail-row.tsx`): grid `grid-cols-[theme(spacing.detail-label)_1fr]` with `px-6 py-4`, label=`text-body text-muted-foreground`, value=`text-label`. `DetailSection` (`Components/ui/detail-section.tsx`): icon+title header + Card wrapper, `noPadding` prop (default false). All 6 detail pages use these. Edge-to-edge dividers pattern: `noPadding` on Section, `p-6` on outer content wrapper, `divide-y -mx-6` on row groups. Pure row sections (Patient/Provider): `noPadding` + `divide-y` directly
...
31. **Health Record Detail**: 4 sidenav sections: Summary, Details, Patient, Provider (no Actions section). Download/Share/linked-entity navigation handled via header dropdown menu, not a dedicated section. All 20+ category detail components use DetailRow for label-value pairs — no paragraph blocks. `VitalsRows` helper renders vitals as DetailRow rows in `divide-y -mx-6`. Pure-row components: `<div className="divide-y">` root. Mixed-content components: `p-6 space-y-6` wrapper + `divide-y -mx-6` for row groups. Interactive elements (follow-up alerts with booking buttons, adherence viz, linked records, lab/PFT tables, discharge instructions) kept as custom layouts
