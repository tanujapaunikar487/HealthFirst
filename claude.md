...
**Detail pages**: `DetailRow` (`Components/ui/detail-row.tsx`): grid `grid-cols-[theme(spacing.detail-label)_1fr]` with `px-6 py-4`, label=`text-body text-muted-foreground`, value=`text-label`. `DetailSection` (`Components/ui/detail-section.tsx`): icon+title header + Card wrapper (`overflow-hidden`), `noPadding` prop (default false). All 6 detail pages use these. `InfoCard` (`Components/ui/info-card.tsx`): used by FamilyMembers/Settings — same label+value layout as DetailRow but in its own bordered card. No avatar prop — name rows show plain text only. Edge-to-edge dividers pattern: `noPadding` on Section, `p-6` on outer content wrapper, `divide-y -mx-6` on row groups. Pure row sections (Patient/Provider): `noPadding` + `divide-y` directly. **No card-in-card**: never `rounded-lg border` inside a Section card — list items use `divide-y` rows (`px-6 py-4`), tables go directly as content (Card clips corners)
...
27. **Financial rows**: Billing fee breakdowns and payment details use `flex justify-between` (NOT DetailRow) — amounts/values right-aligned. `px-6 py-4`, label=`text-body text-muted-foreground`, value=`text-label text-foreground`. Total row label=`text-card-title`, value=`text-subheading`. Same pattern on all 4 financial pages (Billing, Appointments, ClaimDetail, HealthRecords). Payment info rows (Method, Paid on, etc.) use `<DetailRow>` (informational). `bg-muted` sub-headers separate financial from payment sections. **No avatars in detail rows**: `InfoCard` has no avatar prop; `DetailRow` children show plain text for names (Patient/Doctor/Member) — no initials circles
...
31. **Health Record Detail**: Modular architecture — `Show.tsx` (~310 lines) is the layout shell, 15 category section files in `Pages/HealthRecords/sections/`. Types in `types.ts`, category config in `constants.ts`. Common layout: Header → `MetaCard` → `LinkedAppointment` → `SourceSection` → Category sections → Patient → `ActionFooter`. `CategorySection` interface `{id, title, icon, content, action?}` — each `get*Sections()` function returns `CategorySection[]`. `getCategorySections()` router dispatches by `record.category`. 9 new shared UI components: `meta-card`, `linked-appointment`, `source-section`, `vitals-grid`, `specialist-card`, `parameter-card`, `range-bar`, `document-preview`, `action-footer`. `ParameterCard` + `RangeBar` for lab/ECG results with visual normal range. `VitalsGrid` for 4-column vital signs. `SpecialistCard` for doctor/radiologist display. QR codes on vaccination/certificate pages via `qrcode.react`. Collapsible sections for secondary content (hospital course, clinical details, side effects). `pdf-content.ts` imports types from `types.ts` (no duplicate interfaces). Pure-row content: `<div className="divide-y">`. Padded content (tables, alerts): `<div className="p-6">`. Inline tables: `<table className="w-full text-body">` directly as section content (Card `overflow-hidden` clips corners)
