...
18. **Calendar Preference**: `preferred` field in `calendar_sync` setting (`'google'` | `'apple'` | `null`). ConnectionsTab has radio selection for preferred calendar. Confirmation page adapts: Google+connected → "Synced" badge, Apple → direct .ics download, no preference → dropdown with both options. Connecting Google auto-sets preferred; disconnecting clears it.
19. **Toast Styling**: Sonner toasts use `#171717` text (not status-colored), status-colored icons (green/red/blue/amber), content-hugging width via CSS `fit-content`. Configured in `app.tsx` toastOptions + `app.css` data-attribute selectors.
20. **High Contrast Mode**: CSS custom properties in `app.css` (`:root` for defaults, `.high-contrast` for overrides). Tailwind configured with `hsl(var(--*) / <alpha-value>)` syntax in `tailwind.config.js`. All pages, components, and layouts use semantic color classes (`text-foreground`, `bg-background`, `text-muted-foreground`, `border-border`, `bg-primary`, `text-destructive`, etc.) instead of hardcoded hex values. Exception: Razorpay `theme.color` stays as hex (SDK requires raw color string). Toggle via `.high-contrast` class on `<html>` element.
