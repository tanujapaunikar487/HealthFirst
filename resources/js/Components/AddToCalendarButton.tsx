...
export function AddToCalendarButton({
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <Button
        variant={isAdded ? 'primary' : (variant === 'primary' ? 'primary' : 'secondary')}
        size="lg"
        ...
        className={`
          ...
          ${isAdded ? 'bg-success text-white cursor-default hover:bg-success' : ''}
          ${variant === 'secondary' && !isAdded ? 'bg-white text-primary border border-primary hover:bg-primary/5' : ''}
      ...
      {/* Dropdown Menu */}
      {isOpen && !isAdded && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-background rounded-lg shadow-lg border border-border overflow-hidden z-50">
          <div className="py-1">
            <Button
              variant="ghost"
              onClick={() => handleCalendarSelect('google')}
              className="w-full h-auto justify-start rounded-none px-4 py-3 font-normal text-[14px]"
            >
